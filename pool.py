...
from calculations import uniswap_v3
...
class UniswapV3LP:
    ...
    def deposit_amounts(self):
        ...
        return uniswap_v3.deposit_amount(self.pool.token0Price, self.token_0_min_price, self.token_0_max_price, self.amount, self.amount_position)
    
    def tv(self):
        "Calculate the Theorical Value of the LP."
        amount0, amount1 = self.deposit_amounts()
        return uniswap_v3.tv(self.pool.OHLC_df, self.pool.Ticks_df, self.pool.token0Price, self.token_0_min_price, self.token_0_max_price, amount0, amount1, self.pool.volumeUSD, self.pool.FeeTier, self.pool.t0decimals, self.pool.t1decimals)

    def built_ticks(self):
        return uniswap_v3.build_ticks(self.pool.Ticks_df, self.pool.t0decimals, self.pool.t1decimals, self.pool.FeeTier)
