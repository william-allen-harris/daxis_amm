...
def deposit_amount(price: float, price_low: float, price_high:float, amount: float, amount_position: str = 'X') -> Tuple[float, float]:
    """
    ...
    """
    sqrt_upper = sqrt(price_high)
    sqrt_lower = sqrt(price_low)
    sqrt_price = sqrt(price)
    
    if amount_position == 'Y':
        liquidity = amount * sqrt_price * sqrt_upper / (sqrt_upper - sqrt_price)
...
assert deposit_amount(2486.8, 1994.2, 2998.9, 2907.729524805772, 'X') == (2907.729524805772, 1.0000000000000016)
assert deposit_amount(2486.8, 1994.2, 2998.9, 1, 'Y') == (2907.729524805772, 1.0000000000000016)
...
def lp_pool_value(liquidity: float, price: float, upper: float, lower: float) -> float:
    ...
    """
    return 2* liquidity * sqrt(price) - liquidity * (sqrt(lower) + (price/ sqrt(upper)))

assert lp_pool_value(557.9599554712883, 2486.8, 2998.9, 1994.2) == 5394.529524805781
...
assert calc_liquidity(0.12, 513.34, 4029.63, 3635.7, 4443.81) == 159.55760383772108
...
def build_ticks(ticks_df, decimals0, decimals1, fee_tier):
    ...
    """
    tick_list = ticks_df.sort_values("tickIdx", ascending=False).to_dict('records')
    ...
    df['Liquidity_old'] = df['Liquidity'] 
    df['Liquidity'] = df.Liquidity * 10**(decimals0-decimals1)
    return df


def tv(ohlc_df, ticks_df, token_0_price, token_0_lowerprice, token_0_upperprice, amount0, amount1, volume_usd, fee_tier, t0_decimals, t1_decimals):
    ohlc = ohlc_df.copy().iloc[-3*24:]

    average_hr_fees = volume_usd * (fee_tier/1000000) / 24

    liquidity = calc_liquidity(amount0, amount1, token_0_price, token_0_lowerprice, token_0_upperprice)
